...
-- 1. Prevent a trigger failure from aborting refresh-events' batched city upsert.
-- 2. Deduplicate: events that refresh-events deletes and re-inserts (stale-event
--    cleanup) must not re-notify the same user for the same event.
-- 3. Cap per-user notification volume so one ingestion cycle can't flood a user.
-- 4. Index profiles(preferred_city), the trigger's hot lookup column.
