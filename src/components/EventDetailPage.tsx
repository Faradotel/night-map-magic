...
export function EventDetailPage({ event, onClose, userLocation, attendance }: EventDetailPageProps) {
  ...
  const source = event.id.startsWith('tm-') ? 'Ticketmaster' : event.id.startsWith('shotgun-') ? 'Shotgun' : 'NightMap';
  const sourceColor = source === 'Ticketmaster' ? 'hsl(210 100% 56%)' : source === 'Shotgun' ? 'hsl(25 95% 55%)' : 'hsl(var(--primary))';
